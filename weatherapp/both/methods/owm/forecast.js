/* Methods that deal with OpenWeatherMap API for weather forecast are defined here. */

Meteor.methods({
  /* owmForecast: method to get the weather forecast for a city
  ...
  */
  owmForecast: function(owmCityId) {
    console.log('Info: Trying to get forecast for city ' + JSON.stringify(owmCityId) + '.');
    ...
    let url = 'https://api.openweathermap.org/data/2.5/forecast?id=' + encodeURIComponent(owmCityId)
    ...
    HTTP.get(url, function(error, result) {
      ...
      let insertCount = Forecast.insert(entry);
